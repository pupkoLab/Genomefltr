...
def download_genome(output_path: str, taxid: str, acession_numbers: list):
    ...
    res = ngd.download(taxids=[taxid], flat_output=True, output=output_path, file_formats='fasta', assembly_levels='complete',acession_numbers=acession_numbers, use_cache=True)
    print(f'res = {res}')
    if res == 0:
        ...
        res = ngd.download(taxids=[taxid], flat_output=True, output=output_path, file_formats='fasta', acession_numbers=acession_numbers, use_cache=True)
        print(f'res = {res}')
        if res != 0:
            return False
    return True
