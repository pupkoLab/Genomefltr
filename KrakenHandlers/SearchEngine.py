...
from KrakenConsts import BASE_PATH_TO_KRAKEN_SCRIPT, KRAKEN_SEARCH_SCRIPT_COMMAND, KRAKEN_DB_NAMES, \
    KRAKEN_JOB_TEMPLATE, KRAKEN_JOB_QUEUE_NAME, KRAKEN_JOB_ACCOUNT_NAME, NUBMER_OF_CPUS_KRAKEN_SEARCH_JOB, KRAKEN_JOB_PREFIX, CODE_BASE_PATH, \
    KRAKEN_CUSTOM_DB_NAME_PREFIX, CREATE_FILE_FOR_POST_PROCESS_COMMAND, KRAKEN_DB_MEM_REQS, PATH_TO_VERIFY_INPUT_FILE_SCRIPT, \
    KRAKEN_RESULTS_FILE_NAME, OUTPUT_PROCESSOR_JOB_QUEUE_NAME, NUBMER_OF_CPUS_OUTPUT_PROCESSOR_JOB, OUTPUT_PROCESSOR_JOB_PREFIX, \
    OUTPUT_PROCESSOR_JOB_TEMPLATE, FOLDER_PATH_TO_DOWNLOAD_DB
from SharedConsts import PATH_TO_OUTPUT_PROCESSOR_SCRIPT, RESULTS_SUMMARY_FILE_NAME, INPUT_CLASSIFIED_FILE_NAME, \
    ...
    INPUT_CLASSIFIED_FILE_NAME_PAIRED, INPUT_UNCLASSIFIED_FILE_NAME_PAIRED, USER_FILE_NAME, PATH_2_PYTHON_ACTIVATE
import glob
import datetime
...
class SearchEngine:
    ...
    def kraken_search(input_path, run_parameters, db_name='Bacteria'):
        ...
        results_file_path = input_path_parent / KRAKEN_RESULTS_FILE_NAME
        report_path = input_path_parent / RESULTS_SUMMARY_FILE_NAME
        run_parameters = SearchEngine._create_kraken_search_job_text(input_path, run_parameters,
        ...
        logger.info('kraken_search: paramters for submit_job')
        logger.info(run_parameters)
        return submit_job(**run_parameters)

    @staticmethod
    def output_processor(input_path):
        """
        this function actually preforms the kraken2 search
        :param input_path: path to input query file
        :return: created job id, path to results
        ...
        job_unique_id = str(input_path_parent.stem)
        temp_script_path = input_path_parent / f'temp_output_processor_running_file_{job_unique_id}.sh'
        run_parameters = SearchEngine._create_output_processor_job_text(input_path, job_unique_id, results_file_path)
        run_parameters['logger'] = logger
        # run the job
        #with open(temp_script_path, 'w+') as fp:
        #    fp.write(temp_script_text)
        ...
        #job_run_output = subprocess.run(terminal_cmd, stdout=PIPE, stderr=PIPE, shell=True)
        # os.remove(temp_script_path)
        logger.info('output_processor: paramters for submit_job')
        logger.info(run_parameters)
        return submit_job(**run_parameters)

    @staticmethod
    def _create_output_processor_job_text(query_path, job_unique_id, result_path):
        """
        ...
        job_name = f'{OUTPUT_PROCESSOR_JOB_PREFIX}_{job_unique_id}'
        job_logs_path = str(query_path_parent) + '/'
        script_commands = OUTPUT_PROCESSOR_JOB_TEMPLATE.format(
        ...
        return {"queue": OUTPUT_PROCESSOR_JOB_QUEUE_NAME, 
                "account_name": KRAKEN_JOB_ACCOUNT_NAME,
                "num_cpus": NUBMER_OF_CPUS_OUTPUT_PROCESSOR_JOB, 
                "job_name": job_name,
    ...
    def _create_kraken_search_job_text(query_path, run_parameters, job_unique_id, result_path, report_path, db_name):
        ...
        try:
            db_path = SearchEngine._calc_db_folder(db_name)
        except Exception as e:
            logger.error(e)
            db_path = BASE_PATH_TO_KRAKEN_SCRIPT / db_name
        job_logs_path = str(query_path_parent) + '/'
        script_commands = KRAKEN_JOB_TEMPLATE.format(kraken_base_folder=CODE_BASE_PATH,
                                          kraken_command=kraken_run_command, db_path=db_path,
                                          query_path_string=query_path_string,
                                          path_to_folder=query_path_parent,
                                          path_to_input_validator_script=PATH_TO_VERIFY_INPUT_FILE_SCRIPT,
                                          query_path=query_path[0],
        ...
        return {"queue": KRAKEN_JOB_QUEUE_NAME, 
                "account_name": KRAKEN_JOB_ACCOUNT_NAME,
                "num_cpus": NUBMER_OF_CPUS_KRAKEN_SEARCH_JOB, 
                "job_name": job_name,
                "logs_path": job_logs_path,
                "script_commands": script_commands,
                "memory": mem_req}
    ...
    @staticmethod
    def _calc_db_folder(db_name):
        if db_name == 'Bacteria':
            update_db_path = os.path.join(FOLDER_PATH_TO_DOWNLOAD_DB, 'bacteria')
        elif db_name == 'Viral':
            update_db_path = os.path.join(FOLDER_PATH_TO_DOWNLOAD_DB, 'viral')
        else:
            update_db_path = os.path.join(FOLDER_PATH_TO_DOWNLOAD_DB, db_name)
        
        if os.path.isdir(update_db_path):
            list_of_folders = glob.glob(f"{update_db_path}/*")
            ...
            if len(list_of_ok_dir) == 0:
                # run default db
                return BASE_PATH_TO_KRAKEN_SCRIPT / db_name
    ...
    def _db_names_handler(name):
        ...
        if name.find(KRAKEN_CUSTOM_DB_NAME_PREFIX) == -1:
            assert name in list(KRAKEN_DB_MEM_REQS.keys())
