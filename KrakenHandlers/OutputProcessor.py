...
def get_NCBI_renaming_dict(summary_res_df):
    ...
    renaming_dict['A'] = 'Low quality reads (ambiguous nucleotides)'
    ...
    for i, level in zip(range(len(regex_exps)), levels):
        ...
        for chunk in pd.read_csv(outputFilePath, sep='\t', header=None, chunksize=DF_LOADER_CHUCK_SIZE):
            ...
            chunk.loc[chunk['unmapped'] == True, 'taxid'] = chunk.loc[
                chunk['unmapped'] == True, 'max_specie'].str.replace("^(.*)(?=taxid)", "", regex=True).str.replace("[^0-9]", "", regex=True)
            try:
                chunk['taxid'] = chunk['taxid'].astype(int)
            except Exception as e:
                chunk['taxid'] = chunk['taxid'].astype(np.uint64)
            ...
            # actually separate
            unclassified_chunk = pd.concat([chunk[chunk['is_classified'] == 'U'], kraken_mistakes], ignore_index=True)
            ...
            if first:
                ...
                classified_chunk[RESULTS_COLUMNS_TO_KEEP].to_csv(str(processed_Classified_for_PostProcess_results_path),
                                                                 mode='a', header=False, index=False)
                df_preprocess = pd.concat([df_preprocess, df_preprocess_temp], ignore_index=False)
        ...
        try:
            ...
            df_preprocess = df_preprocess.astype(np.uint64)
        ...
        df_preprocess = pd.concat([line, df_preprocess], ignore_index=False)

        df_preprocess.to_csv(processed_for_UI_results_path)

    # it may be possible that the classified_ids and unclassified_ids contain headers from file1 only
    # thus we need to modify them for the paired file too
    # headers for example XXXXX.1.1, XXXX.2.1 should be changed to -->> XXXXX.1, XXXXX.2
    pool_amount = min(1000, len(classified_ids))
    pool_classified = classified_ids[:pool_amount]
    random_amount = min(100, len(classified_ids))
    
    if len(pool_classified):
        random_pool = random.sample(pool_classified, random_amount)
        is_ends_with_same_chars = True
        last_2_chars = str(random_pool[0])[-2:]
        for k in random_pool:
            temp_2_chars = str(k)[-2:]
            if temp_2_chars != last_2_chars:
                is_ends_with_same_chars = False
    else:
        is_ends_with_same_chars = False
    
    # create search lists for SeqKit
    if is_ends_with_same_chars: 
        # in this case, we believe the last char is the read number and the char prior to this is one of the following: [".", "/"]
        digit2add = last_2_chars[0]
        # dont include last two chars
        def create_headers(list_of_headers):
            list2return = []
            for k in list_of_headers:
                k = str(k)[:-2]
                # we add the digit to keep the same format as was used
                list2return.append(k + digit2add + "1")
                list2return.append(k + digit2add + "2")
            return list2return
        classified_ids_string = '\n'.join(create_headers(classified_ids))
        unclassified_ids_string = '\n'.join(create_headers(unclassified_ids))
    else:
        # include last two chars
