...
PATH_TO_TEMP_AWK_PARAM_FILE = Path('var_file.txt')
...
PATH_2_PYTHON_ACTIVATE = "/genomefltr/venv/bin/activate"
...
PATH_TO_OUTPUT_PROCESSOR_SCRIPT = Path("/genomefltr/KrakenHandlers/OutputProcessor.py")
CUSTOM_DB_NAME = 'custom'  # the name of the custom db type
ACCOUNT_NAME = "pupko-users_v2"

SPECIAL_MARTHA_DB_NAME = 'MARTHA'
SPECIAL_MARTHA_JOB_NAME_STRING = "SPECIAL_MARTHA_RUN"
...
INTERVAL_BETWEEN_LISTENER_SAMPLES = 180  # in seconds
INTERVAL_BETWEEN_CLEANING_THE_PROCESSES_DICT = 24  # in hours
TIME_TO_SAVE_PROCESSES_IN_THE_PROCESSES_DICT = 14  # in days
TIME_TO_KEEP_PROCSES_IDS_FOLDERS = 14  # in days the entire folder of the process


# post processing
POSTPROCESS_JOB_PREFIX = 'PP'
POSTPROCESS_JOB_QUEUE_NAME = 'pupko-pool'
POSTPROCESS_JOB_ACCOUNT_NAME = 'pupko-users_v2'
...
cat "$output_path_new_classified_data_temp" | /genomefltr/bin/seqkit grep -f "$output_pathTemp" -v -o "$output_path_new_classified_data_paired"
"""
FILTER_ORIGINAL_CLASSIFIED_RESULTS = 'cat "$original_classified_data_paired" | /genomefltr/bin/seqkit grep -f "$output_pathTemp"  -o "$Temp_new_unclassified_seqs_paired"\n'
...
head -500 "$output_pathTemp" |awk -F "" '{print $(NF-1)$NF}' > "$last_digits_file"\n
uniq -c "$last_digits_file" | [ $(wc -l) -eq 1 ] && (sed 's/2$/1/' "$output_pathTemp" > "$output_pathTemp_first_read"; sed 's/1$/2/' "$output_pathTemp" > "$output_pathTemp_second_read"; cat "$output_pathTemp_second_read" >> "$output_pathTemp_first_read"; mv "$output_pathTemp_first_read" "$output_pathTemp") || echo 'last digits are different'\n
'''

POST_PROCESS_COMMAND_TEMPLATE = '''#!/bin/bash          
...
temp_awk_para_file='{path_to_temp_awk_param_file}'
last_digits_file='{path_to_temp_last_digits_file}'
output_pathTemp_first_read='{path_to_temp_first_read_file}'
output_pathTemp_second_read='{path_to_temp_second_read_file}'


# filter kraken results by query name and threshold
echo "$string" > "{path_to_temp_awk_param_file}"
cat "$input_path" | awk -F "," 'BEGIN{{while ((getline var < "{path_to_temp_awk_param_file}") > 0) split(var, parts, ","); for (i in parts) dict[parts[i]]}} $6 <= {classification_threshold} || !($3 in dict) {{print }}' | awk -F "," 'NR!=1 {{print $2}}' > "$output_pathTemp"

# check if headers are of reads from a single end reads, if so, copy the headers and create for the paired file too
{check_if_lines_suit_paired_file}

# filter original classified results
cat "$original_classified_data" | /genomefltr/bin/seqkit grep -f "$output_pathTemp"  -o "$Temp_new_unclassified_seqs"
...
cat "$output_path_new_classified_data_temp" | /genomefltr/bin/seqkit grep -f "$output_pathTemp" -v -o "$output_path_new_classified_data"
